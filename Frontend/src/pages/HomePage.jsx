...
import { Flag, ChevronDown, Zap } from 'lucide-react'
      ...
      <div className="relative overflow-hidden">
        ...
        <div className="max-w-4xl mx-auto px-4 pt-20 pb-16 text-center relative">
          ...
          <h1 className="text-5xl sm:text-6xl font-black text-white mb-4 tracking-tight">
            F1 Race <span className="text-f1-red">Report</span>
          </h1>
      ...
      <div className="max-w-2xl mx-auto px-4 pb-20">
        <div className="f1-card-accent">
          <h2 className="text-white font-bold text-lg mb-6 flex items-center gap-2">
            <Zap size={18} className="text-f1-red" /> Select a Race
          </h2>
          <div className="space-y-4">
            <div>
              <label className="block text-gray-400 text-sm font-medium mb-2">Season</label>
              <div className="relative">
                <select className="f1-select" value={selectedSeason} onChange={handleSeasonChange} disabled={seasonsLoading}>
                  <option value="">{seasonsLoading ? 'Loading...' : '— Select a season —'}</option>
                  {seasons?.map(s => (
                    <option key={s.season} value={s.season}>{s.season} Formula 1 World Championship</option>
                  ))}
                </select>
            ...
            <div>
              <label className="block text-gray-400 text-sm font-medium mb-2">Grand Prix</label>
              <div className="relative">
                <select className="f1-select" value={selectedRound} onChange={e => setSelectedRound(e.target.value)} disabled={!selectedSeason || racesLoading}>
                  <option value="">
                    {!selectedSeason ? '— Select a season first —' : racesLoading ? 'Loading...' : '— Select a Grand Prix —'}
                  </option>
                  {races?.map(r => (
            ...
            <button className="btn-primary w-full justify-center mt-2 py-4 text-base" onClick={handleGenerate} disabled={!selectedSeason || !selectedRound}>
              <Zap size={18} /> View Race Results
            </button>
          </div>
        </div>
